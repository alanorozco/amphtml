...
function readableRemovalId(id, {percentage, previousHistory}) {
  const {authorDate, hash} = previousHistory[0];
  return `(${truncateYyyyMmDd(authorDate)}, ${hash}) \`${id}\`: ${percentage}`;
}

/**
 * @param {!Array<string>=} files
 */
function maybeLintFix(files) {
  if (argv.skip_lint_fix) {
    log('❗️ (Not fixing lint issues due to --skip_lint_fix)');
    return;
  }
  getOutput(`gulp lint --fix --files="${files.join(',')}"`);
}
